...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .cardwrap .hint { position: absolute; top: 50%; transform: translateY(-50%); font-weight: 800; font-size: calc(var(--base-size) * 0.85px); color: var(--accent); opacity: var(--sw, 0); pointer-events: none; }
    .cardwrap .hint.right { right: 14px; }
    .cardwrap .hint.left { left: 14px; }
    ...
    .ptr { position: fixed; top: 0; left: 0; right: 0; z-index: 30; background: var(--bg); display: flex; align-items: center; justify-content: center; gap: 10px; padding: 12px 0; color: var(--muted); font-size: calc(var(--base-size) * 0.8px); transform: translateY(-100%); transition: transform 0.18s ease; pointer-events: none; }
  ...
  <script>
    ...
    var I18N = {
      fa: { dateLocale: "fa-IR-u-ca-persian", numberLocale: "fa-IR", dir: "rtl", appTitle: "خبر ایران", searchPlaceholder: "جست‌وجوی خبرها...", loading: "در حال دریافت خبرها...", updatedPrefix: "آخرین به‌روزرسانی:", loadFailed: "دریافت خبرها انجام نشد", noNews: "خبری یافت نشد.", now: "اکنون", thread: "رشته‌خبر", parts: "بخش", newLabel: "جدید", settings: "تنظیمات", theme: "پوسته", themeLight: "سفید", themeDark: "تیره", themeComfort: "راحتی چشم", font: "قلم", fontVazirmatn: "وزیرمتن", fontSystem: "سیستم", fontClassic: "کلاسیک", textSize: "اندازه متن", language: "زبان", refresh: "به‌روزرسانی", reset: "بازنشانی", pullHint: "برای به‌روزرسانی بکشید", releaseHint: "رها کنید تا به‌روزرسانی شود", refreshingHint: "در حال به‌روزرسانی...", view: "نمایش", viewFeed: "خبرها", viewArchive: "بایگانی",
        restore: "بازگردانی",
        archiveEmpty: "خبری در بایگانی نیست. برای بایگانی کردن، یک خبر را به چپ یا راست بکشید." },
    ...
    function setupSwipe(card) { var startX = 0, startY = 0, currentX = 0, isSwiping = false; card.addEventListener('touchstart', function (e) { startX = e.touches[0].clientX; startY = e.touches[0].clientY; isSwiping = false; }, {passive: true}); card.addEventListener('touchmove', function (e) { var dx = e.touches[0].clientX - startX; var dy = e.touches[0].clientY - startY; if (!isSwiping && Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 12) { isSwiping = true; card.classList.add('swiping'); ptrStartY = null; ptrSetOffset(0); ptrState = "idle"; } if (isSwiping) { currentX = dx; var translateX = dx * 0.9; card.style.transform = 'translateX(' + translateX + 'px) rotate(' + (translateX * 0.02) + 'deg)'; if (card.parentElement) card.parentElement.style.setProperty('--sw', Math.min(1, Math.abs(dx) / 200).toFixed(2)); } }, {passive: true}); card.addEventListener('touchend', function () { if (isSwiping) { card.classList.remove('swiping'); if (Math.abs(currentX) > 110) { var dir = currentX > 0 ? 1 : -1; card.style.transform = 'translateX(' + (dir * 110) + '%)'; card.style.opacity = '0'; var ids = String(card.dataset.tweetIds || "").split(/\s+/).filter(Boolean); if (viewMode === "feed") { ids.forEach(function (id) { archivedIds[id] = true; }); } else { ids.forEach(function (id) { delete archivedIds[id]; }); } saveArchivedIds(); setTimeout(function () { var rm = (card.parentElement && card.parentElement.classList.contains('cardwrap')) ? card.parentElement : card; rm.remove(); if (document.querySelectorAll('#feed .card').length === 0) render(); }, 220); } else { card.style.transform = ''; if (card.parentElement) card.parentElement.style.setProperty('--sw', '0'); } } isSwiping = false; currentX = 0; }, {passive: true}); }
    function render() { var queryRaw = document.getElementById("search").value.trim(); var query = queryRaw.toLowerCase(); var feed = document.getElementById("feed"); var groups = groupNews(news); groups = groups.map(function (group) { return group.filter(function (item) { var id = String(item.tweet_id || "").trim(); return viewMode === "feed" ? !archivedIds[id] : !!archivedIds[id]; }); }).filter(function (group) { return group.length > 0; }); if (query) { groups = groups.map(function (group) { return group.filter(function (item) { var text = String(item.text || "").toLowerCase(); var tag = String(item.tag || "").toLowerCase(); return text.indexOf(query) !== -1 || tag.indexOf(query) !== -1; }); }).filter(function (group) { return group.length > 0; }); } if (groups.length === 0) { feed.innerHTML = '<div class="empty">' + escapeHtml(viewMode === "archive" ? t("archiveEmpty") : t("noNews")) + '</div>'; return; } var html = ""; for (var gi = 0; gi < groups.length; gi++) { var group = groups[gi]; var latest = group[group.length - 1]; var chips = []; var tweetIds = ""; var hasUnseen = false; var isFresh = false; for (var i = 0; i < group.length; i++) { var id = String(group[i].tweet_id || "").trim(); if (id) tweetIds += (tweetIds ? " " : "") + id; if (id && !seenIds[id]) hasUnseen = true; if (id && !renderedIds[id]) isFresh = true; } if (hasUnseen) chips.push('<span class="chip new">' + escapeHtml(t("newLabel")) + '</span>'); if (latest.tag) chips.push('<span class="chip">' + escapeHtml(latest.tag) + '</span>'); if (group.length > 1) chips.push('<span class="chip soft">' + escapeHtml(t("thread")) + ' · ' + numberFormatter.format(group.length) + ' ' + escapeHtml(t("parts")) + '</span>'); var texts = ""; for (var j = 0; j < group.length; j++) texts += '<div class="text">' + highlightText(group[j].text, queryRaw) + '</div>'; html += '<div class="cardwrap"><div class="hint right">' + escapeHtml(viewMode === "archive" ? t("restore") : t("viewArchive")) + '</div><div class="hint left">' + escapeHtml(viewMode === "archive" ? t("restore") : t("viewArchive")) + '</div><article class="card' + (isFresh ? ' fresh' : '') + '" data-unseen="' + (hasUnseen ? 'true' : 'false') + '" data-tweet-ids="' + escapeHtml(tweetIds) + '">' + (chips.length ? '<div class="chips">' + chips.join("") + '</div>' : '') + texts + '<div class="time">' + escapeHtml(formatTime(latest.created_at)) + '</div></article></div>'; } feed.innerHTML = html; observeUnseenCards(); document.querySelectorAll('.card').forEach(setupSwipe); for (var g2 = 0; g2 < groups.length; g2++) for (var k = 0; k < groups[g2].length; k++) { var id2 = String(groups[g2][k].tweet_id || "").trim(); if (id2) renderedIds[id2] = true; } }
